...
class LinkedList {
    ...
    public: 
        ...
        void check_change_samenumber(int &c_number, int &b_number, int &n) {
            NODE * ptr = this->head;

            for(int i = 0; i != n; i++) {
                if(ptr->data == b_number) {
                    ptr->data = c_number;
                }
                ptr = ptr->next;
            }
...
int main() {
    LinkedList ll;
    int n, data, pos, c_number, b_number;
    std::cin >> n;
    ...
    std::cin >> b_number >> c_number;
    //ll.detele_element(pos, n);
    //ll.Print();
    ll.check_change_samenumber(c_number, b_number, n);
    ll.Print();
    return 0;
}